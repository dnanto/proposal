...
meta <- 
  jqr::jq(
    read_file("data/ACU57015/src.json"), 
...
msa <- toupper(ape::read.dna("data/ACU57015/phylo/msa.fna", format = "fasta", as.character = T))

taxa <-
  map_df(lapply(rownames(msa), function(ele)
    setNames(
      str_match(ele, "(.+)_(\\d{4}-\\d{2}-\\d{2})_(\\d+)$")[1, ],
      c("label", "accver", "date", "taxid")
    )
  ), bind_rows) %>%
  merge(meta, by = "accver", suffixes = c("", ".y"), sort = F) %>%
...
rownames(msa) <- recode(rownames(msa), !!!setNames(taxa$relabel, taxa$label))

tree <- ape::read.tree("data/ACU57015/phylo/cfml.labelled_tree.newick")
tree$tip.label <- recode(tree$tip.label, !!!setNames(taxa$relabel, taxa$label))
...
recom <-
  ape::read.gff("data/ACU57015/phylo/gub.recombination_predictions.gff") %>%
  apply(1, function(row) {
  ...
  bind_rows() %>%
  mutate(accver = factor(recode(accver, !!!setNames(taxa$relabel, taxa$label)), levels = lvls)) %>%
