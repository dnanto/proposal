...
```{r}
meta <- 
  file.path(root, "FJ643676.1", "meta.json") %>%
  read_file() %>%
  jqr::jq(".result | del(.uids) | map([.accessionversion, .taxid, .organism, .subtype, .subname]) |.[]") %>% 
  textConnection() %>% 
...
msa <- 
  file.path(root, "FJ643676.1", "msa-1.fna") %>%
...
tree <- file.path(root, "FJ643676.1", "gub.final_tree.tre") %>% ape::read.tree()
...
recom <-
  file.path(root, "FJ643676.1", "gub.recombination_predictions.gff") %>%
  ape::read.gff() %>%
  apply(1, function(row) {
...
```{r}
res1 <- file.path(root, "FJ643676.1", "clock.poi.rds") %>% readRDS()
res2 <- file.path(root, "FJ643676.1", "clock.rlx.rds") %>% readRDS()
res <- capture.output(modelcompare(res1, res2)) %>% last() %>% str_match("Model (\\d)") %>% last() %>% as.integer()
res <- if(res == 1) res1 else res2
```
...
select(df.est, clock, coalescent, joint, prior, likelihood, age.root., meanRate) 
```

```{r, echo=FALSE, fig.height=10, fig.width=10}
tree.beast <- str_replace(df.bf$path[2], ".mle\\.result\\.log$", ".mcc.tree") %>% read.beast()
