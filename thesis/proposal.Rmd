...
meta <- 
  jqr::jq(
    read_file("../out/ACU57015/meta.json"), 
    ".result | del(.uids) | map([.accessionversion, .taxid, .organism, .subtype, .subname]) |.[]"
  ) %>% 
  textConnection() %>% 
  jsonlite::stream_in(simplifyDataFrame = T, verbose = F) %>%
  setNames(c("accver", "taxid", "organism", "subtype", "subname")) %>%
  bind_cols(
    .,
    bind_rows(
...
msa <- toupper(ape::read.dna("../out/ACU57015/msa-1.fna", format = "fasta", as.character = T))

meta <-
  map_df(lapply(rownames(msa), function(ele)
    setNames(
  ...
  select(label, accver, date, taxid, organism, country) %>%
...
tree <- ape::read.tree("../out/ACU57015/gub.final_tree.tre")
...
recom <-
  ape::read.gff("../out/ACU57015/gub.recombination_predictions.gff") %>%
  apply(1, function(row) {
...
```{r}
res1 <- readRDS("../out/ACU57015/clock.str.rds")
res2 <- readRDS("../out/ACU57015/clock.rlx.rds")
```
...
tree.str <- as_treedata(res1)
tree.str@phylo$tip.label <- recode(tree.str@phylo$tip.label, !!!setNames(meta$relabel, meta$label))
plot_chronogram(tree.str)
```
...
df.bf <-
  list.files("../out/ACU57015", pattern = ".mle.result.log", recursive = T, full.names = T) %>%
...
select(df.bf, clock, coalescent, everything(), -path)
```

```{r}
pburn <- 0.1

list.files("../out/ACU57015", pattern = "\\w+-\\w+.log", recursive = T, full.names = T) %>%
  ...
  mutate_at("path", basename) %>%
  separate(path, c("clock", "coalescent"), sep = "[-.]", extra = "drop")
```

```{r, echo=FALSE, fig.height=10, fig.width=10}
# tree.beast <- read.beast(df.bf$path[1])
tree.beast <- str_replace(df.bf$path[1], ".mle\\.result\\.log$", ".mcc.tree") %>% read.beast()
tree.beast@phylo$tip.label <- recode(tree.beast@phylo$tip.label, !!!setNames(meta$relabel, meta$label))
plot_chronogram(tree.beast)
```
