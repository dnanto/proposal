...
select(df.bf, acc, clock, coalescent, everything(), -path)
```
...
select(df.est, acc, clock, coalescent, joint, prior, likelihood, age.root., meanRate) 
```
```{r}
df.result <-
  filter(df.ess, joint >= 200, prior >= 200, likelihood >= 200) %>%
    select(acc, clock, coalescent) %>%
    merge(df.bf) %>%
    merge(df.est) %>%
    select(acc, clock, coalescent, `BF (PS)`, age.root., meanRate) %>%
    arrange(desc(`BF (PS)`)) %>%
    group_by(acc) %>%
    top_n(1, `BF (PS)`) %>%
    ungroup()
...
```{r, echo=FALSE, fig.height=10, fig.width=10}
tree.beast <- read.beast("../out/FJ643676.1/rex-exp.mcc.tree")
