...
Evaluate clock signal...

```{r}
tre <-
  file.path(params$root, "phy.treefile") %>%
  ape::read.tree()

tre$meta <-
  enframe(tre$tip.label, name = NULL) %>%
  separate(value, c("accver", "range", "collection_date", "taxid"), "_", convert = T) %>%
  mutate_at("collection_date", as.Date)

tre$tip.label <- tre$meta$accver

sts <- with(tre$meta, setNames(as.numeric(collection_date), accver))

s <- ncol(file.path(params$root, "qry.fna") %>% ape::read.dna("fasta", as.character = T))

td <- treedater::dater(tre, sts, s)
treedater::rootToTipRegressionPlot(td)
```
