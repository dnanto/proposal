...
```{r}
plot_beast_tree <- function(path)
{
  tree <- treeio::read.beast(path)
...
with(df.top, file.path(params$root, acc, "beast", str_c(str_c(clock, coalescent, sep = "-"), "mcc.tree", sep = "."))) %>%
  lapply(plot_beast_tree)
```
