...
tree <-
  with(df.top, file.path(params$root, "ACU57010", "beast", str_interp("rln-con.mcc.tree"))) %>%
  treeio::read.beast()
...
ggtree(tree, mrsd = mrsd, right = T) +
  geom_tiplab(size = 2, linesize = 0.25, align = T) +
...
```{r, fig.width=10, fig.height=4}
tree <-
  with(df.top, file.path(params$root, "ACU57015", "beast", str_interp("str-con.mcc.tree"))) %>%
  treeio::read.beast()
...
```{r, fig.width=10, fig.height=4}
tree <-
  with(df.top, file.path(params$root, "ACU57030", "beast", str_interp("str-con.mcc.tree"))) %>%
  treeio::read.beast()
