...
```{r}
keys <- paste0(c("joint", "prior", "likelihood", "age.root.", "meanRate"), ".ess")

df.res <-
  merge(df.est, df.ess, by = c("acc", "clock", "coalescent"), suffixes = c(".est", ".ess")) %>%
  merge(df.bf, ., sort = F) 
...
tree <-
  ...
  .[.$acc == acc, ] %>% 
  head(1) %>%
  with(paste0(clock, "-", coalescent, ".mcc.tree")) %>%
  file.path(root, acc, .) %>%
  read.beast()
...
```{r}
select(df.res, acc, `BF (PS)`, clock, coalescent, starts_with(c("age.root.", "meanRate")), -path, -state) %>%
