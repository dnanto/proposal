...
- Perform phylogeographic and molecular clock analysis for selected adenovirus genotypes.
...
```{r}
select(df.res, clock, coal, starts_with(c("age.root.", "meanRate")), `BF (PS)`, -path, -state) %>%
  knitr::kable("html", col.names = c("clock", "coal", c("est" ,"ess", "est" ,"ess"), "BF (PS)")) %>%
  add_header_above(c("model" = 2, "age(root)" = 2, "mean(rate)" = 2, " " = 1)) %>%
  row_spec(idx, bold = T)
```
